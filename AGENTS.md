...
- Dialogue script: `lib/dialogue/` — `types.ts` holds the transport-free `DialogueScript` (`version: 2`, a list of turns each with `speaker: "system" | "learner"`, `text`, and its own `targetWords`) plus the shared constraint constants (`MIN_TURNS`, `MAX_TARGET_WORDS_PER_TURN`, …); `validate.ts` holds `validateScript(script, requestedWords)`, the **single definition of a valid script**, and the `isDialogueScript` shape guard. Change a rule there, not in the prompt. `words.ts` owns whole-word matching and safe rendering: `containsWord` and `renderHighlightedHtml(text, words)`. Target words are carried as data — never recover them by substring search (`cold` matches inside `colder`). Every `dangerouslySetInnerHTML` site fed model or user text must go through `renderHighlightedHtml` (or `escapeHtml`): it highlights on raw text and escapes per segment, so escaping can neither break a match nor be broken by one. Don't reintroduce `highlightWords(escapeHtml(x))` — that order corrupts entities. `lib/gemini.ts`'s older `highlightWords` survives only inside `DialogueDisplay`'s legacy branch.
- Dialogue generation: `lib/gemini.ts` + `app/api/dialogue/route.ts`. The route asks Gemini for JSON against a `responseSchema`, assigns each turn's `index` from array position, validates, and on a violation retries **exactly once** on the same model with the violations fed back, then returns 422. Model fallback is for transport failures only — a validation failure means the model understood and got it wrong. Only a *validated* rejection may produce a 422; a transport failure on the repair call is a 500. The `violations` array in a 422 body is diagnostic — English, model-facing — and must never be shown to the user; the Vietnamese `error` string is the user-facing text.
- Voice practice: `hooks/useGeminiLive.ts` + `public/audio-processor.worklet.js`.
- History/persistence: `lib/history.ts` — the only persistence layer in the app. `HistoryEntry.script` is the v2 script; entries written before it stored one markdown string and normalize on read to `script: null` + `legacyDialogue`. Consumers branch on `script` and fall back to `legacyDialogue`. In practice a legacy entry only ever reaches `VoicePractice.tsx` (via `/practice?id=…`); `DialogueDisplay.tsx` also keeps a legacy markdown branch, but nothing currently routes an old entry into it — it is retained-but-unreachable, so don't delete it and don't assume it is exercised either. `getAll()` skips stored elements that aren't presentable as entries; `readRaw` stays raw so mutators never rewrite what is already in `localStorage`.
